...
duree_run = 60 #durée du run en min
objectif_qualite = "90%"
durée_moyenne_prod = 300 #durée moyenne pour produire une porte (sec)
lead_time = int(0) #durée avant de produire la première portière (sec)
...
def on_message(client, userdata, message):
    ...
    heure_ini = liste_messages[0]
    heure_ini = heure_ini.split(";")
    print(message)
    ...
    if identifiant_porte[0] == "1" and conformite == "retouche": #avant retouché
        ...
        X_temps_retouches.append(conversion_heure_sec(a[-2]) - conversion_heure_sec(heure_ini[1])/60)
        root.update()
    ...
    if identifiant_porte[0] == "1" and conformite == "rebut": #avant_rebut
        ...
        X_temps_rebuts.append((conversion_heure_sec(a[-2]) - conversion_heure_sec(heure_ini[1]))/60)
        root.update()
    ...
    X_temps.append((conversion_heure_sec(a[-2]) - conversion_heure_sec(heure_ini[1]))/60)
...
def go():
    ...
    coeff = int(objectif_porte)/(duree_run*60)
...
def plot_values():
    ...
    if boutonOK == 1:
        figure.add_subplot(111).plot(X_th,Y_th, color='black',  linewidth=2)
    figure.add_subplot(111).plot(X_temps,Y_prod, color='#172983', linewidth=2, color = "green")
